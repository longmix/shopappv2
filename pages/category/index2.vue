<template>
	<view class="section">	
			...
			<scroll-view scroll-y="true" class="left">
				<view v-for="(item,idx) in types" :key="idx" :data-idx='idx' @tap="tapType" :class="[idx == currType ? 'active' : '']" style="font-size:33rpx；">
					{{item.name}}
				</view>
			</scroll-view>
				...
				<view class="typeTree">
					...
					<block v-if="wxa_product_super_list_style == 1">
						...
						<navigator style="" :url="'../product/detail?productid=' + item.productid"  open-type='navigateTo'>
							...
							<view style="float:right;margin-top: 8rpx;"><image style="padding:0 10px;width:36rpx;height:36rpx;" @tap.prevent="addCart" src="../../static/img/car.png" :data-productid="item.productid"></image></view>
						</navigator>
	...
	export default {
		...
		methods: {
			call_back_set_option:function(that, cb_params){
				...
				that.wxa_product_super_list_style = option_list.wxa_product_super_list_style ? option_list.wxa_product_super_list_style : 0;
				that.wxa_order_super_cata_parentid = option_list.wxa_order_super_cata_parentid ? option_list.wxa_order_super_cata_parentid : 0;
...
.right{ margin-left: 180rpx; height: 100%; overflow: hidden; width: auto;background: #fff;}
.right text{ font-size: 30rpx}
.typeTree{ padding-top: 20rpx;width: 100%}
.typeTree .item{  text-align: center; display: inline-block; vertical-align: top;}
.typeTree .brand,
.typeTree .category{ 
  flex-wrap: wrap; overflow: hidden;display: inline-block;width: 45%;margin-left: 3%;
  margin-bottom: 28rpx;
}
.typeTree .title{ padding: 20rpx 0; display: block; color: #333;}
.typeTree image { width: 200rpx; margin: 0 auto;}
.brand image{ height: 200rpx;}
.typeTree .category image{ height: 810rpx;}
.typeTree .category{ padding-bottom: 20rpx;}
.item text{ 
  padding-bottom: 10rpx; display: block;
  height: 100rpx;
...
.addcart-con{display: flex;justify-content: space-between;align-items: center;}
.addcart-con text{ font-size: 28rpx;color: #f00;}
.addcart-con image{width: 36rpx;height: 36rpx;}
.logo_info {
  margin-left: 18rpx;
