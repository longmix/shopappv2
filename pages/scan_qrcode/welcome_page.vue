<template>
	<view>
		...
		<!-- 排序最大的商品 product_first_item -->
		<view v-if="product_first_item != null">
			<view style="padding: 20rpx;">
				<view style="display: flex;margin: 10px 0;" @tap="tiaozhuan_shop_detail(product_first_item.productid)">
					<view style="width: 35%;">
						<image class="image_shop_gonghuoshang" :src="product_first_item.picture" mode="widthFix"></image>
					</view>
					<view style="padding: 20rpx;width: 60%;">
						<view class="product_first_item_name">{{product_first_item.name}}</view>
						<view class="shop_xiaoshou" style="margin: 10px 0;">
							已销售{{product_first_item.sale_volume}}件</view>
						<view class="shop_price">￥{{product_first_item.price}}元</view>
					</view>
				</view>
				<view class="liji_goumai" @tap="buy_immediately(product_first_item.productid)">立即购买</view>
			</view>
		</view>
	...
	<!-- #endif -->
		...
		<!-- 除了第一个商品之外的其他商品列表" -->
	<view class="product_other_list_gonghuoshang" v-if="product_other_list != null">
			<block v-for="(product_item, index) in product_other_list" :key="index">
		
				<view style="width: 40%;margin: 30rpx;" @tap="tiaozhuan_shop_detail(product_item.productid)">
					<view>
						<image class="image_shop_gonghuoshang" :src="product_item.picture" mode="widthFix"></image>
					</view>
					<view class="shop_name_gonghuoshang">{{product_item.name}}</view>
					<view class="shop_xiaoshou">已销售{{product_item.sale_volume}}件</view>
					<view style="color: #d9001b;">￥{{product_item.price}}</view>
		...
		<!-- 客服功能按钮 -->
		<view v-if="usercenter_contact_status == 17555577">
...
export default {
	...
	data() {
		return {
			...
			//江汉印花定制的功能
			product_other_list:null,
			product_first_item:null,
			factory_item:null,
		};
	},	
	onLoad: function (options) {
		...
		//===== End ======
		console.log('welcome  ====>>1111111111111111', options);
		...
		//this.product_first_item = {};
		//this.product_first_item.sale_volume = 0;
		//this.product_first_item.price = 0;
	...
	methods: {
		...
		__get_share_data:function(){
			...
			if(that.factory_item && that.factory_item.factory_logo){
				share_img = that.factory_item.factory_logo;
			}
			
			
			//分享的标题
			var share_title = this.current_title;
			
			if(that.factory_item && that.factory_item.factory_name){
				share_title = that.factory_item.factory_name;
			}
			
			console.log('onShareAppMessage ==>> ' + share_title);
			console.log('onShareAppMessage ==>> ' + last_url);
			console.log('onShareAppMessage ==>> ' + share_img);
			
			var share_data = {};
			
			share_data.share_title = share_title;
		...
		__load_welcome_page_date:function(options){
			...
			console.log('12121212121215454545454====>>>',parentid);
...
// #endif				
				...
				//2022.12.30. 获取商品列表（第一个商品  和 除了第一个商品之外的其他商品）
				if(http_data.product_first_item){
					that.product_first_item = http_data.product_first_item;
				}
				
				
				if(http_data.product_other_list){
					that.product_other_list = http_data.product_other_list;
				}
			...
			__handle_http_response_factory_item:function(factory_item){
				var that = this;
				if(factory_item){
					that.factory_item = factory_item;
				}
				  	
				uni.setNavigationBarTitle({
					title:that.factory_item.factory_name
				})		
				
			},
			__handle_http_response_product_first_item:function(product_first_item){
				var that = this;
				if(product_first_item){
					that.product_first_item = product_first_item;
				}
			},
			__handle_http_response_product_other_list:function(product_other_list){
				var that = this;
				if(product_other_list){
					that.product_other_list = product_other_list;
				}
			},
		  ...
		  __get_img_from_data_url: function (data_url, that){
		    ...
		    var post_data = {
		      openid: this.abotapi.get_current_openid()
				...
				var http_data = uni.getStorageSync('welcome_page_data_cache_' + that.http_data_cache_id);
				var factory_item = uni.getStorageSync('welcome_page_factory_item_cache_' + that.http_data_cache_id);
				var product_first_item = uni.getStorageSync('welcome_page_factory_item_cache_' + that.http_data_cache_id);
				var product_other_list = uni.getStorageSync('welcome_page_product_other_list_cache_' + that.http_data_cache_id);
				
				if(factory_item){
					
					that.__handle_http_response_factory_item(factory_item);
					
				}
				if(product_first_item){
					
					that.__handle_http_response_product_first_item(product_first_item);
					
				}
				if(product_other_list){
					
					that.__handle_http_response_product_other_list(product_other_list);
		    ...
		    var cbSuccess = function (res) {
		      ...
		      if (res.data.code == 1) {
		        //var wz_keyword = res.data.data.keyword;
					...
					that.__handle_http_response_data(res.data.data);
					that.__handle_http_response_factory_item(res.data.factory_item);
					that.__handle_http_response_product_first_item(res.data.product_first_item);
					that.__handle_http_response_product_other_list(res.data.product_other_list);
					...
					uni.setStorage({
						key: 'welcome_page_factory_item_cache_' + that.http_data_cache_id,
						data: res.data.factory_item
					})
				
					uni.setStorage({
						key: 'welcome_page_product_first_item_cache_' + that.http_data_cache_id,
						data: res.data.product_first_item
					})
				
					uni.setStorage({
						key: 'welcome_page_product_other_list_cache_' + that.http_data_cache_id,
						data: res.data.product_other_list
					})
		...
		tiaozhuan_shop_detail:function(productid){
			var that = this;
			uni.navigateTo({
				url:'/pages/product/detail?productid='+ productid ,
			})
		},
		buy_immediately:function(productid){
			var that = this;
			uni.navigateTo({
				url:'/pages/order/pay?amount=1&productid=' + productid +'&action=direct_buy'
			})
...
<style>
	...
	.product_other_list_gonghuoshang{
		display:flex;
		flex-wrap: wrap;
		padding: 20rpx;
	}
