<template>
	<view class="bg_content">
		<button @click="copyToClipboard(input_txt)" size="mini" v-if="textarea_flag" class="btn_copy">复制</button>
		<textarea :disabled="textarea_flag" v-model="input_txt" :style="{height:convert_Height+'rpx'}"></textarea>
		<button @click="convertButton" type="primary" id="yourButtonId" ref="myButton"
			class="convert_btn">转为{{ convertingOption === 0 ? '火星文' : '简体字' }}</button>
	</view>
</template>
...
<script>
	export default {
		...
		onLoad() {
			...
			// 获取屏幕高度
			let screenHeight = systemInfo.windowHeight * 2 - 232;
			this.convert_Height = screenHeight


			console.log('屏幕高度', this.sHeight)

		},
		methods: {
			convertButton() {
				// 判断是简体还是火星文
				if (this.convertingOption === 0) {
					this.input_txt = this.qqlized(this.input_txt);
					this.convertingOption = 2;
					// 判断复制按钮的显示和隐藏
					if (this.input_txt === '') {
					...
					this.input_txt = this.simplized(this.input_txt);
					this.convertingOption = 0;
					this.textarea_flag = false
				}
			},
			...
			traditionalized(cc) {
				...
				for (var i = 0; i < cc.length; i++) {
					if (charPYStr().indexOf(cc.charAt(i)) != -1) str += ftPYStr().charAt(charPYStr().indexOf(cc.charAt(
						i)));
					else if (qqPYStr().indexOf(cc.charAt(i)) != -1) str += ftPYStr().charAt(qqPYStr().indexOf(cc.charAt(
						i)));
					else {
						str += cc.charAt(i);
					}
			...
			simplized(cc) {
				var str = '';
				for (var i = 0; i < cc.length; i++) {
					if (this.ftPYStr().indexOf(cc.charAt(i)) != -1) str += this.charPYStr().charAt(this.ftPYStr().indexOf(
						cc.charAt(i)));
					else if (this.qqPYStr().indexOf(cc.charAt(i)) != -1) str += this.charPYStr().charAt(this.qqPYStr()
						.indexOf(cc.charAt(
							i)));
					else {
						str += cc.charAt(i);
			...
			qqlized(cc) {
				var str = '';
				for (var i = 0; i < cc.length; i++) {
					if (this.ftPYStr().indexOf(cc.charAt(i)) != -1) str += this.qqPYStr().charAt(this.ftPYStr().indexOf(cc
						.charAt(i)));
					else if (this.charPYStr().indexOf(cc.charAt(i)) != -1) str += this.qqPYStr().charAt(this.charPYStr()
						.indexOf(cc
							.charAt(i)));
					else {
						str += cc.charAt(i);
