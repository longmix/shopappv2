<template>
	<view class="content">
		...
		<!-- <view  v-if="is_there_list_data == 1"> -->
			<view  v-for="(item , index) in worker_list_list" :key="index">
				<view class="content-list">
					<view class="content-list-hangyi">
						<image src="https://yanyubao.tseo.cn/Tpl/static/images/jiazhenggongsi/客户.png" class="content-list-img" @tap="toWorker_detail(item.citizenid)"></image>
						<text class="content-list-name" @tap="toWorker00_detail(item.citizenid)">{{item.worker_name}}</text>
					...
					<view class="content-list-hangwu">
						<image src="https://yanyubao.tseo.cn/Tpl/static/images/jiazhenggongsi/银行卡.png" class="content-list-shenfen"></image>
						<text class="content-list-phone">银行卡号:&nbsp;&nbsp;{{item.credit_card_numbers}}</text>
					</view>
					<view class="content-list-hangliu">
						<image src="https://yanyubao.tseo.cn/Tpl/static/images/jiazhenggongsi/参加工作时间.png" class="content-list-shenfen"></image>
						<text class="content-list-phone">工作时间:&nbsp;&nbsp;{{item.working_hours}}</text>
					</view>
				</view>
			</view>
		<!-- </view> -->
			...
			<view id="open-modal" class="modal-window">
				...
				<view>
					<view class="modal-zhuti">
						...
						<view class="modal-disan"  @tap="toWorker01_detail(citizenid)">
							<text>查看详细信息</text>
	...
	export default {
		...
		onReachBottom() {
			...
			if(this.current_page != 2){
				...
				//分页
				this._worker_list(this.current_cata_name);
			};
		},
		...
		methods: {
			...
			callback_function_shop_option_data: function(that, cb_params) {
				...
				show_more_option:function(citizenid, citizen_name){
					 ...
					 console.log('asdadsad==>>>>',citizenid)
					 var that = this;
				...
				modal_edit_edit:function(){
					var new_url = '/pages/jiazhenggongsi/worker_edit';
					if(this.current_citizenid){
						new_url += '?citizenid='+this.current_citizenid;
						console.log('999999999999999999999=====>>>>>>>>>>>>>',new_url)
					}
				...
				search_key:function(){
					...
					var post_end_point = '/openapi/JiazhenggongsiData/worker_list';
					var post_data = {
						sellerid: that.abotapi.get_sellerid(),
				...
				jiazhengApi.requestServer(post_end_point, post_data, function(res){
					...
					that.worker_list_list = res.data.worker_list;
				...
				toWorker00_detail:function(citizenid){
					var new_url = '/pages/jiazhenggongsi/worker_detail?citizenid='+citizenid;
					this.abotapi.call_h5browser_or_other_goto_url(new_url);
				},
				toWorker01_detail:function(citizenid){
					var new_url = '/pages/jiazhenggongsi/worker_detail';
					if(this.current_citizenid){
						new_url += '?citizenid='+this.current_citizenid;
					}
					this.abotapi.call_h5browser_or_other_goto_url(new_url);
				...
				_worker_list: function() {	
					...
					jiazhengApi.requestServer(post_end_point, post_data, function(res){
						console.log('/openapi/JiazhenggongsiData/worker_list===>>>', res);
						
						
						if(!that.worker_list_list){
							that.worker_list_list = [];
						}
						
						for(var i=0;i<res.data.worker_list.length;i++){
							console.log('asdasda==sssss=>>>',res.data.worker_list[i])
							
							that.worker_list_list.push(res.data.worker_list[i]);
						};
						
						console.log('asdasda===>>>',that.worker_list_list)
