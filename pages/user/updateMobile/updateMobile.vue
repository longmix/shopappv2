<template>
	<view class="container">
		...
		<view class="section1" style="border: 0; background-color: #ffffff;" v-if="current_mobile">
			当前手机号：{{current_mobile}}
		</view>

		<view class="section1">
			<input type="number" placeholder="请输入新手机号" v-model="mobile" auto-focus />
		</view>
		<view class="section3">
		...
		<view class="section2">
			<input type="number" placeholder="收到的验证码" v-model="tel" auto-focus />
		...
		<button type="primary" @tap="btn_set_user_mobile" class="weui-btn"
			:style="{background:wxa_shop_nav_bg_color, color:wxa_shop_nav_font_color}">设置手机号码</button>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.current) {
				this.current_mobile = options.current;
			}
		...
		methods: {
			...
			btn_set_user_mobile: function() {
				var that = this
				var userInfo = that.abotapi.get_user_info();

				that.abotapi.abotRequest({
					url: that.abotapi.globalData.yanyubao_server_url + '/?g=Yanyubao&m=ShopApp&a=set_user_mobile',
					data: {
			...
			send_btn: function(e) {
				...
				//发送验证码短信
				that.abotapi.abotRequest({
					url: that.abotapi.globalData.yanyubao_server_url + '/?g=Yanyubao&m=ShopApp&a=sendsms_change_mobile',
					data: {
						sellerid: that.abotapi.get_sellerid(),
						userid: userInfo.userid,
						checkstr: userInfo.checkstr,
