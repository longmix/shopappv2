			...
			<view v-if="wxa_login_only_weixin != 1">
				...
				<div class="flex mgb-20">
					<div class="cl-black pointer flex-1" 
						style="color: #a2a2a2;font-size: 26rpx;">Email首次登录自动注册</div>
				</div>
				<div class="flex mgb-20">
					<navigator class="cl-black pointer flex-1" 
						open-type="navigate" 
						url="/pages/login/login_by_password">无法使用Email？账号密码登录</navigator>
				</div>
			</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			email_address_input:function(e){
				//console.log('Phone=',e)
				this.email_address = e.detail.value;
			},		
			imgInput: function (e) {
			...
			verifycode_email_input: function (e) {
				//console.log('tel=' + e.detail.value)
				this.verifycode_email = e.detail.value;
			},
			...
			send_btn:function(){
				...
				that.abotapi.abotRequest({
					url: this.abotapi.globalData.yanyubao_server_url + '/?g=Yanyubao&m=ShopAppWxa&a=send_email',
					data: {
			...
			btn_user_login: function (e) {
				...
				var login_url = that.abotapi.globalData.yanyubao_server_url + '/?g=Yanyubao&m=ShopApp&a=login_by_email_verifycode';
				var post_data = {
						email_address: that.email_address,
						verifycode_email: that.verifycode_email,
				...
				//#ifdef MP-WEIXIN
					...
					login_url = that.abotapi.globalData.yanyubao_server_url + '/?g=Yanyubao&m=ShopAppWxa&a=login_by_email_verifycode';
				...
				//#ifdef MP-ALIPAY
					login_url = that.abotapi.globalData.yanyubao_server_url + '/?g=Yanyubao&m=ShopAppMpAlipay&a=login_by_email_verifycode';
