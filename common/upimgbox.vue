			...
			upImg:function(){
				...
				uni.chooseImage({
					success: function (chooseImageRes) {
						...
						for(var i=0;i<len;i++){
							uni.uploadFile({
								url: this.abotapi.apiHost+'?fromapp=wxapp&m=upload&a=img&ajax=1&authcode='+this.abotapi.getAuthCode(), //仅为示例，非真实的接口地址
								...
								success: function (res) {
									...
									if(!res.data.error){
										...
										that.imgsList=this.abotapi.json_add(that.imgsList,json);
										if(that.imgsData!=""){
